...
public:
    ...
    void remove(int x) {
        ...
        if (target.isleaf) {
            ...
            if (target.node.size() >= 3) {
                deletenleaf(target, x);
            }
            if(target.node.size() <= 2){
                for (int i = 0; i < target.node.size(); i++) {
                    if (x == target.node[i].key) {
                        //search for leaf of target
                        //get parent()
                        //sibling =get_sibling(node , targetnode[i])
                        if(sibling.size()<=2){
                            //merge with sibling
                        }
                        else{
    ...
    int deletenleaf(Node& x , int key ){
        ...
        if(!x.isleaf){
            for(auto & i : x.node){
                if(i.key == key){
                    int replace = deletenleaf(reinterpret_cast<Node &>(i.next), key);
                    i.key = replace;
                    //update index file
                    return replace;
                }
